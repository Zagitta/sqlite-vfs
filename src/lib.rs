...
pub trait DatabaseHandle: Sync {
    ...
    /// Return the sector size of this handle
    fn get_sector_size(&self) -> Result<i32, std::io::Error> {
        Ok(1024)
    }
...
/// Registration handle to a Vfs
pub struct Registration(*mut sqlite3_vfs);

impl Drop for Registration {
    fn drop(&mut self) {
        unsafe {
            ffi::sqlite3_vfs_unregister(self.0);
            drop(Box::<sqlite3_vfs>::from_raw(self.0));
        }
    }
}
